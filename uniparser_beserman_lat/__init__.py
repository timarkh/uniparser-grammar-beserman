...
class BesermanLatAnalyzer(Analyzer):
    def __init__(self, mode='default', verbose_grammar=False):
        """
        Initialize the analyzer by reading the grammar files.
        If mode=='default' (default), load the data as is.
        ...
        if mode not in ('default'):
            return
        self.dirName = 'uniparser_beserman_lat.data_' + mode
    ...
    def analyze_words(self, words, format=None, disambiguate=False):
        ...
        """
        if disambiguate:
            with as_file(files(self.dirName) / 'beserman.cg3') as cgFile:
